...
async function initialize() {
    const networkUrl = core.getInput("network-url", { required: true });
    const multisigPda = core.getInput("multisig-pda", { required: true });
    const multisigVaultIndex = core.getInput("multisig-vault-index", {
        required: true,
    });
    const programId = core.getInput("program-id", { required: true });
    const buffer = core.getInput("buffer", { required: true });
    const spillAddress = core.getInput("spill-address", {
        required: true,
    });
    const name = core.getInput("name", { required: true });
    const keypair = core.getInput("keypair", { required: true });
